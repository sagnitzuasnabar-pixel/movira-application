...
import { useGetPeliculasByGenre } from '../hooks/use-get-peliculas-by-genre'
import { useSliderNavigation } from '../../../../../hooks/use-slider-navigation'

const IMAGE_BASE_URL_SMALL = 'https://image.tmdb.org/t/p/w500'

export function PeliculasRow({ title, genreId }) {
  const { peliculas, loading, error } = useGetPeliculasByGenre(genreId)
  ...
  return (
    <section className="px-4 md:px-8 py-4">
      ...
      <div className="relative group">
        ...
        >
          {peliculas.map(function(movie) {
            return (
              <div
                key={movie.id} onClick={() => { handleClickMovie(movie.id) }}
